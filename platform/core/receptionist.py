...
async def create_receptionist_pipeline(
    ...
    # --- Telnyx serializer ---
    telnyx_config = config.get("telnyx", {})
    ...
    # Register call with guards
    registered = await guards.register_call(call_id, from_number, pipeline_task=task)
    if not registered:
        logger.warning(f"[receptionist] Could not register call {call_id}, aborting")
        return
