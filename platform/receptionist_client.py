...
async def run_bot(websocket, stream_sid, call_sid="", account_sid="", from_number="unknown"):
    ...
    system_prompt += (
        ...
        "Step 8: If yes, say 'What's your email? Go ahead and spell it out for me so I get it right.' "
        "Wait for them to spell it. As they spell, build the email exactly from the letters they say — do NOT guess, autocorrect, or fill in anything yourself. "
        "Common clarifications to listen for: 'at' means @, 'dot' means ., 'underscore' means _. "
        "If they say a letter that sounds ambiguous (like S vs Z, M vs N, B vs D), ask: 'Was that S as in Sam, or Z as in Zebra?' before moving on. "
        "Once you have the full email, read it back letter by letter: 'Let me read that back — [spell every character]. Did I get that right?' "
        "If they correct anything, fix it and read the full email back again. "
        "Only send with send_confirmation_email once they say yes it's correct. "
...
<div id="addModal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.8);z-index:100;padding:20px;overflow-y:auto">
  <div style="max-width:400px;margin:40px auto;background:#1a1a24;border:1px solid #2a2a3a;border-radius:16px;padding:24px">
    ...
    <select id="addDate" onchange="loadAddSlots()" style="width:100%;padding:10px 12px;border-radius:8px;border:1px solid #2a2a3a;background:#0a0a0a;color:#fff;font-size:0.9em;margin-bottom:8px;font-weight:700"><option value="" style="color:#fff;font-weight:700">Pick a Day</option></select>
    <select id="addTime" style="width:100%;padding:10px 12px;border-radius:8px;border:1px solid #2a2a3a;background:#0a0a0a;color:#fff;font-size:0.9em;margin-bottom:8px;font-weight:700"><option value="" style="color:#fff;font-weight:700">Pick a Time (EST)</option></select>
...
function renderAppts() {{
  ...
  container.innerHTML = filtered.map(a => {{
    ...
    return '<div class="card' + (isToday ? ' today-card' : '') + '"><div class="top"><span class="name">' + (a.name||'Unknown') + '</span><span class="time-badge">' + (a.time||'TBD') + '</span></div><div class="top" style="margin-bottom:0"><span style="font-size:0.78em;color:#999">' + (a.service||'') + '</span>' + paidBadge + '</div><div style="margin-top:6px;font-size:0.9em;font-weight:700;color:#f59e0b">Potential: $' + pot.toLocaleString() + '</div><div class="details" style="margin-top:4px">' + (a.phone ? '<div>&#128222; <span>' + a.phone + '</span></div>' : '') + (a.notes ? '<div style="margin-top:6px;padding:8px;background:rgba(255,255,255,0.04);border-radius:8px;font-size:0.82em;white-space:pre-wrap;word-break:break-word;color:#ccc;line-height:1.5">&#128221; ' + a.notes + '</div>' : '') + '</div>' + meetLink + actions + '</div>';
  }}).join('');
}}
...
function loadAddSlots() {{
  ...
  for (let h = 9; h < 17; h++) {{
    ...
    if (taken) {{ o.value = ''; o.textContent = label + ' — Reserved'; o.disabled = true; o.style.color = '#777'; o.style.fontWeight = '400'; }}
