"""
Janovum AI Receptionist — Simple Twilio Server
Based on official Pipecat examples that actually work.
"""
...
# Your public tunnel URL — update this each time tunnel restarts
PUBLIC_URL = os.environ.get("PUBLIC_URL", "adaptation-meaning-fan-insulin.trycloudflare.com")

# SAFETY: Never make outbound calls or dial emergency numbers
BLOCKED_OUTBOUND = True

# --- Daily limits ---
DAILY_SPEND_CAP = 5.00        # $5/day max
DAILY_CALL_LIMIT = 50         # 50 calls/day max
COST_PER_MINUTE = 0.023       # combined cost per minute
...
class DailyTracker:
    ...
    def check_new_day(self):
        if date.today() != self.date:
            logger.info(f"[limits] New day — resetting. Yesterday: {self.call_count} calls, ${self.total_cost:.2f} spent")
            self.reset()
    ...
    def register_call(self):
        self.check_new_day()
        self.call_count += 1
        logger.info(f"[limits] Call #{self.call_count} today, ${self.total_cost:.2f} spent so far")

    def add_minutes(self, minutes):
        self.total_minutes += minutes
        self.total_cost += minutes * COST_PER_MINUTE
        logger.info(f"[limits] +{minutes:.1f} min, total today: {self.total_minutes:.1f} min, ${self.total_cost:.2f}")
...
async def incoming_call(request: Request):
    """Twilio calls this when someone dials your number. Returns TwiML."""
    ...
    if not can_accept:
        logger.warning(f"[incoming] Rejecting call from {from_number}: {reason}")
        twiml = """<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Say voice="Polly.Joanna">We're sorry, our receptionist is unavailable right now. Please try again tomorrow. Thank you!</Say>
  <Hangup/>
</Response>"""
...
<Response>
  ...
  <Pause length="40"/>
    ...
    logger.info(f"[incoming] Call from {from_number}, streaming to wss://{PUBLIC_URL}/ws")
    return Response(content=twiml, media_type="application/xml")


@app.websocket("/ws")
async def websocket_endpoint(websocket: WebSocket):
    """Twilio streams audio here. We run the Pipecat pipeline."""
    await websocket.accept()
    logger.info("[ws] WebSocket accepted")

    try:
        # Read Twilio's 2 handshake messages
        start_data = websocket.iter_text()
        first_msg = await start_data.__anext__()  # "connected" event — discard
        logger.info(f"[ws] Got connected event")

        second_msg = await start_data.__anext__()  # "start" event — parse streamSid
        call_data = json.loads(second_msg)
        stream_sid = call_data["start"]["streamSid"]
        call_sid = call_data["start"].get("callSid", "")
        account_sid = call_data["start"].get("accountSid", "AC2d50767d64e32c3b57b56a57c11c3849")
        logger.info(f"[ws] Got start event: stream_sid={stream_sid}, call_sid={call_sid}")
        ...
        # Track how long the call lasted
        call_duration = (time.time() - call_start) / 60.0 if 'call_start' in dir() else 0
        daily_tracker.add_minutes(call_duration)
        logger.info(f"[ws] Call ended. Duration: {call_duration:.1f} min")


def check_availability(date_str, time_str):
    """Check if a time slot is available."""
    appts_path = PLATFORM_DIR / "data" / "appointments.json"
...
def save_appointment(appointment):
    """Save an appointment to appointments.json."""
    appts_path = PLATFORM_DIR / "data" / "appointments.json"
    ...
    logger.info(f"[appointments] Saved: {appointment['name']} on {appointment['date']} at {appointment['time']}")
...
def send_sms(to_number, message):
    ...
    try:
        resp = requests.post(
            f"https://api.twilio.com/2010-04-01/Accounts/AC2d50767d64e32c3b57b56a57c11c3849/Messages.json",
            auth=("AC2d50767d64e32c3b57b56a57c11c3849", "94c74007b62652b5b14c7bea70a5792c"),
            data={"From": "+18339589975", "To": to_number, "Body": message},
        )
        logger.info(f"[sms] Sent to {to_number}: status={resp.status_code}")
...
async def run_bot(websocket, stream_sid, call_sid="", account_sid="", from_number="unknown"):
    ...
    transport = FastAPIWebsocketTransport(
        ...
        params=FastAPIWebsocketParams(
            ...
            serializer=TwilioFrameSerializer(
                ...
                auth_token="94c74007b62652b5b14c7bea70a5792c",
            ),
        ),
    )

    # STT — Deepgram (fast, handles phone audio perfectly)
    from pipecat.services.deepgram.stt import DeepgramSTTService
    stt = DeepgramSTTService(
        api_key="6e304c8a16d16deae3ec7694e60212c4f610ba96",
        audio_passthrough=True,
    )

    # LLM — Groq (free, ~200ms response time)
    llm = OpenAILLMService(
        api_key="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",
        model="llama-3.3-70b-versatile",
        base_url="https://api.groq.com/openai/v1",
    )

    # TTS — Cartesia Sonic (40ms latency, sounds real)
    ...
    tools = [
        ...
        },
        {
            ...
            "function": {
                ...
                "description": "Check if a specific date and time is available for booking. ALWAYS check before booking.",
                "parameters": {
                    "type": "object",
                    "properties": {
        ...
        },
        {
            ...
            "function": {
                ...
                "description": "Send appointment confirmation via text message to the caller's phone",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "phone_number": {"type": "string", "description": "Phone number to send to"},
                        "name": {"type": "string", "description": "Caller's name"},
                        "date": {"type": "string", "description": "Appointment date"},
                        "time": {"type": "string", "description": "Appointment time"},
                        "service": {"type": "string", "description": "Service booked"},
    ...
    async def handle_send_email(function_name, tool_call_id, arguments, llm, context, result_callback):
        ...
        success = send_email(args["email"], "Appointment Confirmation - Janovum", body)
        await result_callback({"status": "sent" if success else "failed"})
    ...
    async def handle_check_time_slot(function_name, tool_call_id, arguments, llm, context, result_callback):
        ...
        if available:
            ...
            await result_callback({"available": False, "message": f"That time is already booked. Please suggest a different time."})
    ...
    async def handle_book_appointment(function_name, tool_call_id, arguments, llm, context, result_callback):
        ...
        if not available:
            await result_callback({"status": "conflict", "message": "That time slot is already booked. Ask the caller for a different time."})
            return
    ...
    messages = [
        {
            "role": "system",
            "content": (
                "You are the AI receptionist for Janovum, an AI agency. "
                "You sound exactly like a real human receptionist on the phone. "
                ...
                "Your job: greet callers, book appointments, answer questions about the business. "
    ...
    # When client connects, have the bot introduce itself
    @transport.event_handler("on_client_connected")
    async def on_client_connected(transport, client):
        logger.info("[bot] Client connected — sending greeting")
        messages.append({"role": "system", "content": "Greet the caller. Say something like: Hi there, thanks for calling Janovum! How can I help you today?"})
    ...
    # Run
    logger.info("[bot] Starting pipeline...")
...
async def status():
    return {
        "status": "running",
        "service": "Janovum AI Receptionist",
...
async def get_appointments():
    ...
    if appts_path.exists():
        ...
        appts = []
    return {"total": len(appts), "appointments": appts}

@app.get("/app")
async def appointments_app():
    """Mobile-friendly appointments dashboard."""
    html = """<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<meta name="apple-mobile-web-app-title" content="Janovum">
<title>Janovum — Schedule</title>
<style>
...
.card { background: #1a1a24; border: 1px solid #2a2a3a; border-radius: 10px; padding: 14px; margin-bottom: 8px; transition: border-color 0.2s; }
...
.week-header { padding: 12px 16px 4px; font-size: 0.75em; color: #555; text-transform: uppercase; letter-spacing: 1px; }
</style>
</head>
<body>
<div class="header">
...
function matchDate(apptDate, targetDate) {
  ...
  const target = targetDate.toLowerCase();
  const appt = apptDate.toLowerCase();
  if (appt === target) return true;
  if (appt.includes('today') && target === dateStr(new Date())) return true;
  if (appt.includes('tomorrow')) {
    const tom = new Date(); tom.setDate(tom.getDate()+1);
    return target === dateStr(tom);
  }
  try {
    const parsed = new Date(appt);
    if (!isNaN(parsed)) return dateStr(parsed) === target;
  } catch(e) {}
...
function countForDate(date) {
  ...
  return allAppts.filter(a => a.status === 'confirmed' && matchDate(a.date, ds)).length;
}
...
function renderTabs() {
  ...
  tabs.innerHTML = days.map((d, i) => {
    ...
    return '<div class="day-tab' + (isActive ? ' active' : '') + (isToday ? ' today' : '') + (hasAppts ? ' has-appts' : '') + '" onclick="selectDay(\\''+ds+'\\', '+ i +')">' +
      '<div class="day-name">' + (isToday ? 'Today' : (i===1 ? 'Tmrw' : dayNames[d.getDay()])) + '</div>' +
      '<div class="day-num">' + d.getDate() + '</div>' +
      '<div class="day-count">' + (count > 0 ? count + ' appt' + (count>1?'s':'') : '-') + '</div>' +
    '</div>';
  }).join('');
}

function selectDay(ds, idx) {
  selectedDate = ds;
  renderTabs();
  renderAppts();
}
...
function renderAppts() {
  ...
  document.getElementById('dateLabel').textContent = d.toLocaleDateString('en-US', options);
  ...
  if (filtered.length === 0) {
    container.innerHTML = '<div class="empty">' + (isToday ? 'No appointments today. When someone calls and books, it shows up here.' : 'No appointments scheduled for this day.') + '</div>';
    return;
  }
  ...
  container.innerHTML = filtered.map(a => {
    return '<div class="card' + (isToday ? ' today-card' : '') + '">' +
      '<div class="top">' +
        '<span class="name">' + (a.name||'Unknown') + '</span>' +
        '<span class="time-badge">' + (a.time||'TBD') + '</span>' +
      '</div>' +
      '<div class="top" style="margin-bottom:0">' +
        '<span style="font-size:0.78em;color:#999">' + (a.service||'') + '</span>' +
        '<span class="badge confirmed">CONFIRMED</span>' +
      '</div>' +
      '<div class="details" style="margin-top:6px">' +
        (a.phone ? '<div>&#128222; <span>' + a.phone + '</span></div>' : '') +
        (a.notes ? '<div>&#128221; <span>' + a.notes + '</span></div>' : '') +
      '</div>' +
    '</div>';
  }).join('');
}
...
async function loadData() {
  try {
    ...
    const todayStr = dateStr(new Date());
    const tomStr = dateStr(new Date(Date.now() + 86400000));
    const todayCount = allAppts.filter(a => a.status==='confirmed' && matchDate(a.date, todayStr)).length;
    const tomCount = allAppts.filter(a => a.status==='confirmed' && matchDate(a.date, tomStr)).length;

    let weekCount = 0;
    getWeekDates().forEach(d => { weekCount += countForDate(d); });

    document.getElementById('todayCount').textContent = todayCount;
    document.getElementById('tomorrowCount').textContent = tomCount;
...
async def get_today_appointments():
    ...
    return {"date": today, "total": len(today_appts), "appointments": today_appts}


if __name__ == "__main__":
    print()
    print("=" * 55)
    print("  JANOVUM AI RECEPTIONIST")
    print("=" * 55)
    print(f"  Tunnel: {PUBLIC_URL}")
    print(f"  Server: http://localhost:5051")
    print(f"  Voice:  Cartesia Sonic")
    print(f"  STT:    faster-whisper (local)")
    print(f"  LLM:    Pollinations (free)")
    print("=" * 55)
    print()
