...
@app.route("/api/chat/register-creation", methods=["POST"])
def chat_register_creation():
    """Auto-register tools, employees, workflows, knowledge created via chat terminal."""
    data = request.json or {}
    creation_type = data.get("type", "")
    user_msg = data.get("user_message", "")
    ai_response = data.get("ai_response", "")

    if not creation_type or not ai_response:
        return jsonify({"registered": False, "error": "Missing type or ai_response"})

    try:
        if creation_type == "tool":
            # Extract tool name from context
            name = _extract_name(user_msg, ai_response, "tool")
            desc = ai_response[:200].replace("\n", " ").strip()
            # Save as a custom skill
            skills_dir = os.path.join(PLATFORM_DIR, "data", "custom_skills")
            os.makedirs(skills_dir, exist_ok=True)
            skill_data = {"name": name, "description": desc, "source": "chat", "created_at": datetime.now().isoformat(), "code": ai_response}
            with open(os.path.join(skills_dir, f"{name}.json"), "w") as f:
                json.dump(skill_data, f, indent=2)
            return jsonify({"registered": True, "name": name, "type": "tool"})

        elif creation_type == "employee":
            name = _extract_name(user_msg, ai_response, "employee")
            bots_dir = os.path.join(PLATFORM_DIR, "data", "custom_employees")
            os.makedirs(bots_dir, exist_ok=True)
            emp_data = {"name": name, "description": ai_response[:300], "source": "chat", "created_at": datetime.now().isoformat(), "system_prompt": ai_response}
            with open(os.path.join(bots_dir, f"{name}.json"), "w") as f:
                json.dump(emp_data, f, indent=2)
            return jsonify({"registered": True, "name": name, "type": "employee"})

        elif creation_type == "workflow":
            name = _extract_name(user_msg, ai_response, "workflow")
            wf_dir = os.path.join(PLATFORM_DIR, "data", "workflows")
            os.makedirs(wf_dir, exist_ok=True)
            wf_data = {"name": name, "description": user_msg[:200], "source": "chat", "created_at": datetime.now().isoformat(), "steps_raw": ai_response}
            with open(os.path.join(wf_dir, f"{name}.json"), "w") as f:
                json.dump(wf_data, f, indent=2)
            return jsonify({"registered": True, "name": name, "type": "workflow"})

        elif creation_type == "knowledge":
            kb_dir = os.path.join(PLATFORM_DIR, "data", "knowledge")
            os.makedirs(kb_dir, exist_ok=True)
            title = user_msg[:80].strip()
            kb_data = {"title": title, "content": ai_response, "topic": "chat", "source": "chat", "created_at": datetime.now().isoformat()}
            safe_name = title.replace(" ", "_")[:40]
            with open(os.path.join(kb_dir, f"{safe_name}.json"), "w") as f:
                json.dump(kb_data, f, indent=2)
            return jsonify({"registered": True, "name": title, "type": "knowledge"})

        return jsonify({"registered": False, "error": f"Unknown type: {creation_type}"})
    except Exception as e:
        return jsonify({"registered": False, "error": str(e)})


def _extract_name(user_msg, ai_response, thing_type):
    """Try to extract a reasonable name from the user message."""
    import re
    msg = user_msg.lower()
    # Remove common prefixes
    for prefix in ["create a ", "build a ", "make a ", "create an ", "build an ", "make an ", "hire a ", "hire an "]:
        if msg.startswith(prefix):
            msg = msg[len(prefix):]
            break
    for prefix in [f"{thing_type} that ", f"{thing_type} called ", f"{thing_type} named ", f"{thing_type} to ", f"{thing_type} for ", f"{thing_type} which "]:
        if prefix in msg:
            msg = msg.split(prefix)[1]
            break
    # Clean up and make a snake_case name
    name = re.sub(r'[^a-z0-9\s]', '', msg)[:60].strip()
    name = "_".join(name.split()[:6])
    return name or f"custom_{thing_type}_{int(time.time())}"
...
def api_tools_summary():
    """Return tool summary — total count, categories, and all tool names."""
    try:
...
def api_tools_execute():
    ...
    try:
        ...
        result = execute_tool(tool_name, tool_input, client_id=client_id)
...
def api_tools_categories():
    """Return tools grouped by category with tool names per category."""
    try:
        categories = get_tools_by_category()
        summary = {}
        for cat_name, tools in categories.items():
            summary[cat_name] = [t["name"] for t in tools]
        return jsonify(summary)
    except Exception as e:
        return jsonify({"error": str(e)}), 500


@app.route("/api/tools/detailed")
def api_tools_detailed():
    """Return full tool details grouped by category — name, description, params."""
    try:
        categories = get_tools_by_category()
        result = {}
        for cat_name, tools in categories.items():
...
@app.route("/api/mcp/registry/search")
def api_mcp_registry_search():
    """Proxy search to the real MCP registry — 10,000+ public servers."""
    import requests as _req
    ...
    try:
        ...
        r = _req.get("https://registry.modelcontextprotocol.io/v0/servers", params=params, timeout=10)
        data = r.json()
        servers = []
...
def api_employees_list():
    """List all bot_setup_* tools with their descriptions and input schemas."""
    try:
        all_tools = get_all_tools()
        employees = []
        for t in all_tools:
            if t["name"].startswith("bot_setup_"):
                employees.append({
                    "name": t["name"],
                    "bot_type": t["name"].replace("bot_setup_", ""),
                    "description": t.get("description", ""),
...
@app.route("/api/employees/setup", methods=["POST"])
def api_employees_setup():
    """Set up a new employee bot. Accepts {bot_type, ...config}."""
    try:
        data = request.json or {}
        bot_type = data.pop("bot_type", None)
        if not bot_type:
            return jsonify({"error": "bot_type is required"}), 400
        tool_name = f"bot_setup_{bot_type}"
        if tool_name not in get_tool_names():
            return jsonify({"error": f"Unknown bot type: {bot_type}"}), 404
        result = execute_tool(tool_name, data)
        return jsonify({"bot_type": bot_type, "result": result})
    except Exception as e:
        return jsonify({"error": str(e)}), 500


@app.route("/api/employees/active")
def api_employees_active():
    """List all active employee bots."""
    try:
        result = execute_tool("bot_list", {})
        return jsonify({"active_bots": result})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_knowledge_list():
    ...
    try:
        result = execute_tool("knowledge_list", {})
...
def api_knowledge_save():
    ...
    try:
        data = request.json or {}
        result = execute_tool("knowledge_save", data)
...
def api_knowledge_search():
    """Search knowledge base. Query param: ?q=xxx"""
    try:
        q = request.args.get("q", "")
        if not q:
            return jsonify({"error": "Query parameter 'q' is required"}), 400
        result = execute_tool("knowledge_search", {"query": q})
        return jsonify({"query": q, "results": result})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_workflows_list():
    """List all workflows."""
    try:
        result = execute_tool("workflow_list", {})
...
def api_workflows_create():
    ...
    try:
        data = request.json or {}
        result = execute_tool("workflow_create", data)
        return jsonify({"result": result})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_workflows_run():
    ...
    try:
        data = request.json or {}
        result = execute_tool("workflow_run", data)
        return jsonify({"result": result})
    except Exception as e:
        return jsonify({"error": str(e)}), 500


@app.route("/api/agents/parallel")
def api_agents_parallel():
    """List all parallel agents."""
    try:
        result = execute_tool("agent_list", {})
...
def api_agents_spawn():
    ...
    try:
        data = request.json or {}
        result = execute_tool("agent_spawn", data)
        return jsonify({"result": result})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_agents_team():
    ...
    try:
        data = request.json or {}
        result = execute_tool("agent_team", data)
        return jsonify({"result": result})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_crypto_prices():
    """Get crypto prices. Query param: ?tokens=BTC,ETH"""
    try:
        tokens_str = request.args.get("tokens", "BTC")
        tokens = [t.strip() for t in tokens_str.split(",") if t.strip()]
        result = execute_tool("crypto_price", {"tokens": tokens})
        return jsonify({"tokens": tokens, "prices": result})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_payments_create():
    ...
    try:
        data = request.json or {}
        result = execute_tool("payment_create_invoice", data)
        return jsonify({"result": result})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def serve_contract(contract_id):
    """Serve a signable contract page"""
    html_path = os.path.join(CONTRACTS_DIR, f"{contract_id}.html")
    if not os.path.exists(html_path):
        return "<h1>Contract not found</h1><p>This contract link is invalid or has expired.</p>", 404
    with open(html_path, "r", encoding="utf-8") as f:
        return f.read()

@app.route('/api/contracts/sign', methods=['POST'])
def sign_contract():
    """Handle contract signature submission"""
    try:
        payload = request.json
        cid = payload.get("contract_id", "unknown")

        # Save signed data
        signed_path = os.path.join(CONTRACTS_DIR, f"{cid}_signed.json")
        payload["received_at"] = datetime.now().isoformat()
        with open(signed_path, "w") as f:
            json.dump(payload, f, indent=2)

        # Update contract metadata
        meta_path = os.path.join(CONTRACTS_DIR, f"{cid}.json")
        if os.path.exists(meta_path):
            with open(meta_path, "r") as f:
                meta = json.load(f)
            meta["status"] = "signed"
            meta["signed_at"] = payload["received_at"]
            meta["signer_name"] = payload.get("client_name", "")
            meta["signer_title"] = payload.get("client_title", "")
            with open(meta_path, "w") as f:
        ...
        try:
            ...
            msg["Subject"] = f"CONTRACT SIGNED — {payload.get('client_business', 'Client')}"
...
<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
  <div style="background:#0a0a0a;padding:24px;text-align:center;">
    <h1 style="color:#D4AF37;margin:0;">CONTRACT SIGNED</h1>
  </div>
  <div style="padding:24px;background:#fff;">
    <h2 style="color:#00C853;">New client locked in!</h2>
    <table style="width:100%;border-collapse:collapse;">
      <tr><td style="padding:8px;color:#666;">Client:</td><td style="padding:8px;font-weight:bold;">{payload.get('client_name','')}</td></tr>
      <tr><td style="padding:8px;color:#666;">Business:</td><td style="padding:8px;font-weight:bold;">{payload.get('client_business','')}</td></tr>
      <tr><td style="padding:8px;color:#666;">Title:</td><td style="padding:8px;">{payload.get('client_title','')}</td></tr>
      <tr><td style="padding:8px;color:#666;">Email:</td><td style="padding:8px;">{payload.get('client_email','')}</td></tr>
      <tr><td style="padding:8px;color:#666;">Contract ID:</td><td style="padding:8px;font-family:monospace;">{cid}</td></tr>
      <tr><td style="padding:8px;color:#666;">Signed at:</td><td style="padding:8px;">{payload.get('signed_at','')}</td></tr>
    </table>
    <p style="margin-top:16px;color:#444;">Signature image and full signed data saved to server.</p>
  </div>
</div>
"""
            ...
            with smtplib.SMTP("smtp.gmail.com", 587) as server:
                server.starttls()
                server.login("myfriendlyagent12@gmail.com", "pdcvjroclstugncx")
                server.sendmail("myfriendlyagent12@gmail.com", "myfriendlyagent12@gmail.com", msg.as_string())
            print(f"[+] Contract signed notification sent for {cid}")
        except Exception as e:
        ...
        try:
            ...
            if client_email:
                ...
                conf["Subject"] = f"Janovum — Agreement Confirmed"
                conf["From"] = "Janovum <myfriendlyagent12@gmail.com>"
                conf["To"] = client_email

                conf_html = f"""
<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
  <div style="background:#0a0a0a;padding:24px;text-align:center;">
    <h1 style="color:#D4AF37;margin:0;">JANOVUM</h1>
  </div>
  <div style="padding:24px;background:#fff;">
    <h2>Agreement Confirmed!</h2>
    <p>Hi {payload.get('client_name','')},</p>
    <p>Your service agreement has been received and confirmed. Our team will begin setting up your services right away.</p>
    <p><strong>Contract ID:</strong> {cid}</p>
    <p><strong>What happens next:</strong></p>
    <ul>
      <li>We'll send you an onboarding form within 24 hours</li>
      <li>Your AI services will be configured within 3-5 business days</li>
      <li>You'll receive dashboard access credentials by email</li>
    </ul>
    <p>Questions? Call us at <strong>+1 (833) 958-9975</strong> or reply to this email.</p>
    <p style="margin-top:24px;">Welcome aboard!<br><strong>— The Janovum Team</strong></p>
  </div>
</div>
"""
                conf.attach(MIMEText(conf_html, "html"))
                with smtplib.SMTP("smtp.gmail.com", 587) as server:
                    server.starttls()
                    server.login("myfriendlyagent12@gmail.com", "pdcvjroclstugncx")
                    server.sendmail("myfriendlyagent12@gmail.com", client_email, conf.as_string())
        except Exception as e:
            print(f"[!] Client confirmation email failed: {e}")

        return jsonify({"status": "ok", "message": "Contract signed successfully"})
...
def list_contracts():
    ...
    for f in os.listdir(CONTRACTS_DIR):
        if f.endswith('.json') and not f.endswith('_signed.json'):
            with open(os.path.join(CONTRACTS_DIR, f), 'r') as fp:
                contracts.append(json.load(fp))
