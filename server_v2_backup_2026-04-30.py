...
(function() {
    ...
    var mocks = {
        ...
        'router/capabilities': null,
        'guardrails/rules': null,
...
# ═══ SALES TRACKER USER API ════════════════════════════════════════════
from datetime import datetime as _dt

def _sales_accts():
    path = os.path.join(PLATFORM_DIR, "data", "sales_accounts.json")
    try:
        with open(path) as f: return json.load(f)
    except: return {}

def _sales_accts_save(accts):
    path = os.path.join(PLATFORM_DIR, "data", "sales_accounts.json")
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, "w") as f: json.dump(accts, f)

def _sales_udata(username):
    path = os.path.join(PLATFORM_DIR, "data", f"sales_{username}.json")
    try:
        with open(path) as f: return json.load(f)
    except: return {"products": [], "sales": [], "clients": []}

def _sales_udata_save(username, udata):
    path = os.path.join(PLATFORM_DIR, "data", f"sales_{username}.json")
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, "w") as f: json.dump(udata, f)

def _sales_auth_check(d):
    username = (d.get("username") or "").strip().lower()
    password = d.get("pass", "")
    accts = _sales_accts()
    if username in accts and accts[username].get("pass") == password:
        return username
    return None

@app.route("/api/sales/auth", methods=["POST"])
def sales_auth():
    d = request.json or {}
    action = d.get("action", "login")
    username = (d.get("username") or "").strip().lower()
    password = d.get("pass", "")
    if not username or len(username) < 2 or not password:
        return jsonify({"ok": False, "error": "Missing fields"}), 400
    accts = _sales_accts()
    if action == "signup":
        if username in accts:
            return jsonify({"ok": False, "error": "Username taken"}), 400
        display = (d.get("displayName") or "").strip() or username
        accts[username] = {"pass": password, "displayName": display, "created": _dt.utcnow().isoformat()}
        _sales_accts_save(accts)
        _sales_udata_save(username, {"products": [], "sales": [], "clients": []})
        return jsonify({"ok": True, "displayName": display})
    else:
        if username not in accts:
            return jsonify({"ok": False, "error": "Account not found"}), 401
        if accts[username].get("pass") != password:
            return jsonify({"ok": False, "error": "Wrong password"}), 401
        return jsonify({"ok": True, "displayName": accts[username].get("displayName", username)})

@app.route("/api/sales/data", methods=["POST"])
def sales_get_data():
    d = request.json or {}
    username = _sales_auth_check(d)
    if not username:
        return jsonify({"error": "Unauthorized"}), 401
    return jsonify(_sales_udata(username))
...
def sales_save_data():
    ...
    _sales_udata_save(username, udata)
    return jsonify({"ok": True})

@app.route("/api/sales/accounts", methods=["GET"])
def sales_accounts_list():
    accts = _sales_accts()
    return jsonify([{"username": u, "displayName": v.get("displayName", u)} for u, v in accts.items()])
